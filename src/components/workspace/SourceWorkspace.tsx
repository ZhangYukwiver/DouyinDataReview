      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, mobile && styles.scrollContentMobile]} showsVerticalScrollIndicator={false}>
        <View style={[styles.layout, mobile && styles.layoutMobile]}>
          <View style={[styles.intro, mobile && styles.introMobile]}>
            <Text {...fx({ motion: "rise", i: 1 })} style={styles.eyebrow}>{copy.eyebrow}</Text>
            <Text {...fx({ motion: "rise", i: 2 })} style={[styles.title, phone && styles.titlePhone]}>{copy.title[0]}{phone ? "\n" : ""}{copy.title[1]}</Text>
          ...
          <View style={styles.operations}>
            ...
            <View {...fx({ motion: "rise", i: 5 })} style={styles.utility}><Pressable {...fx({ hover: "tint" })} disabled={!connected || busy || switchingAccount} onPress={onSwitchAccount} style={({ pressed }) => [styles.utilityButton, pressed && styles.pressed, pointer]}><UserRoundCog color={color.textMuted} size={16} /><Text style={styles.utilityText}>切换账号</Text></Pressable>{Platform.OS === "web" ? <Pressable {...fx({ hover: "tint" })} disabled={!total} onPress={onExportData} style={({ pressed }) => [styles.utilityButton, pressed && styles.pressed, pointer]}><Download color={color.textMuted} size={16} /><Text style={styles.utilityText}>导出数据</Text></Pressable> : null}<Pressable {...fx({ hover: "tint" })} disabled={!total || busy} onPress={onClearCache} style={({ pressed }) => [styles.utilityButton, pressed && styles.pressed, pointer]}><Trash2 color={color.textMuted} size={16} /><Text style={styles.utilityText}>清除本地记录</Text></Pressable><Text style={styles.utilityNote}>Cookie 与记录只保存在当前设备</Text></View>
...
function Count({ icon: CountIcon, label, value }: { icon: React.ComponentType<{ color?: string; size?: number }>; label: string; value: number }) { const shown = useCountUp(value); return <View style={styles.count}><CountIcon color={color.accent} size={18} /><View><Text style={styles.countValue}>{shown.toLocaleString("zh-CN")}</Text><Text style={styles.countLabel}>{label}</Text></View></View>; }
function ActionButton({ busy, disabled, icon: ActionIcon, label, onPress }: { busy?: boolean; disabled: boolean; icon: React.ComponentType<{ color?: string; size?: number }>; label: string; onPress: () => void }) { return <Pressable {...fx({ hover: "raise" })} accessibilityRole="button" disabled={disabled} onPress={onPress} style={({ pressed }) => [styles.action, disabled && styles.disabled, pressed && styles.pressed, pointer]}>{busy ? <ActivityIndicator color={color.accent} size="small" /> : <ActionIcon color={color.textSecondary} size={17} />}<Text style={styles.actionText}>{label}</Text></Pressable>; }
...
const styles = StyleSheet.create({
  ...
  archive: { flexDirection: "row", alignItems: "center", gap: 11, marginTop: 24, paddingTop: 20, borderTopWidth: 1, borderTopColor: color.border }, archiveMobile: { alignItems: "flex-start" }, archiveIcon: { width: 37, height: 37, alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: color.frame, borderRadius: radius.small }, archiveButton: { minHeight: 38, flexDirection: "row", alignItems: "center", gap: 7, paddingHorizontal: 11, borderWidth: 1, borderColor: color.frame, borderRadius: radius.pill }, archiveButtonText: { color: color.accent, fontSize: 10, fontWeight: "800" }, utility: { flexDirection: "row", flexWrap: "wrap", alignItems: "center", gap: 9, marginTop: 18 }, utilityButton: { minHeight: 34, flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 9, borderWidth: 1, borderColor: color.borderSoft, borderRadius: radius.pill }, utilityText: { color: color.textMuted, fontSize: 9 }, utilityNote: { flex: 1, minWidth: 150, color: color.textMuted, fontSize: 9, textAlign: "right" },
